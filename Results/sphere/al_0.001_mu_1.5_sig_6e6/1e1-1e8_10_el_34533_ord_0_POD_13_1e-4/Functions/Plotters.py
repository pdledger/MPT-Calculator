...
def PODInvPlotter(savename,Array,PODArray,EigenValues,PODEigenValues,EddyCurrentTest):
    ...
    #Setup data
    Inv=EigenValues
    PODInv=PODEigenValues

    Points = len(Array)
    #Overwrite with Invairnats
    for i in range(Points):
        Inv[i,1]=EigenValues[:,0]+EigenValues[i,1]+EigenValues[i,2]
        Inv[i,2]=(EigenValues[i,0].real*EigenValues[i,1].real+EigenValues[i,0].real*EigenValues[i,2].real+EigenValues[i,1].real*EigenValues[i,2].real)+1j*(EigenValues[i,0].imag*EigenValues[i,1].imag+EigenValues[i,0].imag*EigenValues[i,2].imag+EigenValues[i,1].imag*EigenValues[i,2].imag)
        Inv[i,3]=(EigenValues[i,0].real*EigenValues[i,1].real*EigenValues[i,2].real)+1j*(EigenValues[i,0].imag*EigenValues[i,1].imag*EigenValues[i,2].imag)
        PODInv[i,1]=PODEigenValues[:,0]+PODEigenValues[i,1]+PODEigenValues[i,2]
        PODInv[i,2]=(PODEigenValues[i,0].real*PODEigenValues[i,1].real+PODEigenValues[i,0].real*PODEigenValues[i,2].real+PODEigenValues[i,1].real*PODEigenValues[i,2].real)+1j*(PODEigenValues[i,0].imag*PODEigenValues[i,1].imag+PODEigenValues[i,0].imag*PODEigenValues[i,2].imag+PODEigenValues[i,1].imag*PODEigenValues[i,2].imag)
        PODInv[i,3]=(PODEigenValues[i,0].real*PODEigenValues[i,1].real*PODEigenValues[i,2].real)+1j*(PODEigenValues[i,0].imag*PODEigenValues[i,1].imag*PODEigenValues[i,2].imag)
    ...
    for i,line in enumerate(ETP):
        if i==0:
            lines = ax.plot(Array,Inv[:,line-1].real,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,Inv[:,line-1].real,MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i,line in enumerate(ETP):
        lines += ax.plot(PODArray,PODInv[:,line-1].real,SLS,markersize=SMS,color=PYCOL[i])
    ...
    #Label the axes
    plt.xlabel("Frequency (rad/s)")
    plt.ylabel(r"$I_i(\mathcal{N}^0+\mathcal{R})$")

    #Title
    if Title==True:
        plt.title(r"Invairants $\mathcal{N}^0+\mathcal{R}$")

    #Create the legend
    names = []
    for i,number in enumerate(ETP):
        names.append(r"$I_{"+str(number)+"}(\mathcal{N}^0+\mathcal{R})$ (POD)")
    for i,number in enumerate(ETP):
        names.append(r"$I_{"+str(number)+"}(\mathcal{N}^0+\mathcal{R})$ (Snapshot)")
    ...
    #Save the graph
    plt.savefig(savename+"RealInv.pdf")
    ...
    for i,line in enumerate(ETP):
        if i==0:
            lines = ax.plot(Array,Inv[:,line-1].imag,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,Inv[:,line-1].imag,MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i,line in enumerate(ETP):
        lines += ax.plot(PODArray,PODInv[:,line-1].imag,SLS,markersize=SMS,color=PYCOL[i])
    ...
    #Label the axes
    plt.xlabel("Frequency (rad/s)")
    plt.ylabel(r"$I_i(\mathcal{I})$")

    #Title
    if Title==True:
        plt.title(r"Invairants of $\mathcal{I}$")

    #Create the legend
    names = []
    for i,number in enumerate(ETP):
        names.append(r"$I_{"+str(number)+"}(\mathcal{I})$ (POD)")
    for i,number in enumerate(ETP):
        names.append(r"$I_{"+str(number)+"}(\mathcal{I})$ (Snapshot)")
    ...
    #Save the graph
    plt.savefig(savename+"ImaginaryInv.pdf")
