...
def PODInvPlotter(savename,Array,PODArray,EigenValues,PODEigenValues,EddyCurrentTest):
    ...
    for i,line in enumerate(ETP):
        if i==0:
            lines = ax.plot(Array,np.abs(Inv[:,line-1].real),MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,np.abs(Inv[:,line-1].real),MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i,line in enumerate(ETP):
        lines += ax.plot(PODArray,np.abs(PODInv[:,line-1].real),SLS,markersize=SMS,color=PYCOL[i])
    ...
    for i,line in enumerate(ETP):
        if i==0:
            lines = ax.plot(Array,np.abs(Inv[:,line-1].imag),MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,np.abs(Inv[:,line-1].imag),MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i,line in enumerate(ETP):
        lines += ax.plot(PODArray,np.abs(PODInv[:,line-1].imag),SLS,markersize=SMS,color=PYCOL[i])
