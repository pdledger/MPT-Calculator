...
def Checkvalid_new(Object,Order,alpha,inorout,mur,sig,Stepmesh):
    Object = Object[:-4]+".vol"
    ...
    Volmesh=Integrate(1,mesh)*(alpha**3)
    print("Volume of the mesh is",Volmesh)

    Volair=Integrate((1-inout),mesh)*(alpha**3)
    print("Volume of air is",Volair)

    print("Volume of air and object - mesh",Volmesh-Volair-Vol)
    ...
    ndof=femfull.ndof
#   count how many boundary surfaces
#    npp=0
...
#    noutput=npp
#    print("Number of boundary faces = ",npp)
...
#        sol.Set(exp(-((x-list[i,0])**2 + (y-list[i,1])**2 + (z-list[i,2])**2)/sval**2),definedon=mesh.Boundaries("default"))
        k=2.*np.pi/(1/(i+1))
        sol.Set(sin(k*x)*sin(k*y)*sin(k*z),definedon=mesh.Boundaries("default"))
    ...
    print(" maximum recomeneded frequency is ",str(round(cond))[:-2])
